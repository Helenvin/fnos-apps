...
/**
 * 解析歌单/专辑/单曲 URL，返回完整歌曲列表（List<Music>，含 bits/plugName 可直接下载）
 * 后端 ParserController.parserUrl 无 @RequestBody，采用 query 绑定
 */
export function parserUrlSongs(url) {
    return request({
        url: baseUrl + parserUrl+"/parserUrl",
        method: "post",
        params: {"url":url,"isAudioBook":false}
    })
