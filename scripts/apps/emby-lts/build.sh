...
echo "==> Building Emby-LTS ${VERSION} (${DEB_ARCH})"
...
# 预置插件：apps/emby-lts/plugins/<插件名>/… → 应用内 plugins-default/
if [ -d apps/emby-lts/plugins ] && [ -n "$(ls -A apps/emby-lts/plugins 2>/dev/null)" ]; then
  mkdir -p "$dst/plugins-default"
  tar -cf - -C apps/emby-lts/plugins \
...
cp apps/emby-lts/fnos/EmbyServer.sc "$dst/"
cp apps/emby-lts/fnos/config/* "$dst/config/"
cp -a apps/emby-lts/fnos/ui/* "$dst/ui/"
cp apps/emby-lts/fnos/bin/emby-server "$dst/bin/emby-server"
